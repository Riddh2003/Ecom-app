...
public class CustomerDao {
    ...
    public void insertCustomer(CustomerBean bean) {
        stmt.update("insert into customer(firstname, gender, email, password, role) values(?,?,?,?,?)",
                bean.getFirstname(),
                bean.getGender(), bean.getEmail(), bean.getPassword(), bean.getRole());
    }

    public CustomerBean authenticateCustomerByEmail(String email) {
        CustomerBean customer = stmt.queryForObject("select * from customer where email = ?",
                new BeanPropertyRowMapper<>(CustomerBean.class), email);
        return customer;
    }
